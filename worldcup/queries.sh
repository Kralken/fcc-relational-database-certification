...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games AS winner ON teams.team_id = winner.winner_id LEFT JOIN games AS opponent ON teams.team_id = opponent.opponent_id WHERE (winner.year = 2014 AND winner.round='Eighth-Final') OR (opponent.year = 2014 AND opponent.round = 'Eighth-Final')")"
